...
/* ----------------------------------------------------- */
                if(isset($_POST['fechaini']) && isset($_POST['fechafin'])){
                        ...
                        switch ($tipotrab) {
                                ...
                                break;
                                case 'EXTRACANON':
                                        $sql_consulta = "SELECT tf.* FROM tblExtraCanon tx, tblEstaciones te, tblFormulario tf WHERE tx.idFormulario = tf.idFormulario AND tx.idEstacion = te.idEstacion AND te.idRegional = $idregion ORDER BY tf.fecha_ini_intervalo ASC;"; 
                                break;
                                default:
                                                if($idregion > 0){
         ...
         /*        if($sql_consulta===false){
                        ...
                        if($count_consulta_g!=false){
                                        ...
                                        while($row_arriba=sqlsrv_fetch_array($query_arriba)){
                                                ...
                                                $contenido_tabla="";
                                                $contenido_tabla2="";
                                                $tipo_formulario = $row_arriba['tipo_formulario'];
                                                $id_formulario=$row_arriba['idFormulario'];
                                                $consulta_formulario="SELECT te.idEstacion,te.id,te.nombre_Estacion,tf.tipo_est,tf.depto,tf.provincia,tf.mun_sec,tf.localidad,te.sistema,tipo_trayecto,tramo_1,tramo_2 FROM tblFormulario tf,tblEstaciones te WHERE tf.idEstacion=te.idEstacion AND tf.idFormulario=$id_formulario";
                                                // echo '<br>'.$consulta_formulario;
                                                ...
                                                while($row_formulario=sqlsrv_fetch_array($query_formulario)){ 
                                                      ...
                                                      /*   $contenido_tabla.= "<td>".$id_formulario."</td>"; */
                                                        $contenido_tabla.= "<td>".utf8_decode($row_formulario['id'])."</td>";
                                                        ...
                                                        $contenido_tabla2.= "<td>".utf8_decode($row_formulario['tipo_trayecto'])."</td>";
                                                        $contenido_tabla2.= "<td>".utf8_decode($row_formulario['tramo_1'])."</td>";
                                                        $contenido_tabla2.= "<td>".utf8_decode($row_formulario['tramo_2'])."</td>";
                                                        $contenido_tabla2.= "<td>Obs en trabajo</td>";  
                                                }
                                                // echo '<br> ---->'.$contenido_tabla;
                                                                ...
                                                                /* correctivos */
                                                                        ...
                                                                        if($count_correctivo!=false){
                                                                                while($row_correctivo=sqlsrv_fetch_array($query_correctivo)){
                                                                                        set_time_limit(25);
                                                                                        // echo '<br>Buscando '.$id_formulario.', correc: '.$sql_pre;
                                                                ...
                                                                /* EXTRACANON */
                                                                        ...
                                                                        $count_preventivo=sqlsrv_has_rows($query_preventivo);  
                                                                        if($count_preventivo!=false){
                                                                                // echo '<br>Buscando '.$id_formulario.', preven: '.$sql_pre;
                                                                                while($row_preventivo=sqlsrv_fetch_array($query_preventivo)){
                                                                                        set_time_limit(25);
                                                                                        $fecha_inicial=$row_preventivo['fechaInicio']->format('Y-m-d'); 
                                                                                        $hora_inicial=$row_preventivo['horaInicio'];
                                                                        ...
                                                                        if($tamaño_vector==0){
                                                                                ...
                                                                                if($tama_vector == 0){
                                                                                        ...
                                                                                        for ($i=0; $i < $tama_vector; $i++) { 
                                                                                                // if(isset($vector_equipo_reemplazo[$i]) && strlen(trim($vector_equipo_reemplazo[$i]))>0 && isset($vector_modelo_retirado[$i]) && strlen(trim($vector_modelo_retirado[$i]))>0 && isset($vector_serie_retirado[$i]) && strlen(trim($vector_serie_retirado[$i]))>0 && isset($vector_modelo_reemplazo[$i]) && strlen(trim($vector_modelo_reemplazo[$i]))>0 && isset($vector_serie_reemplazo[$i]) && strlen(trim($vector_serie_reemplazo[$i]))>0 && isset($vector_observaciones[$i]) && strlen(trim($vector_observaciones[$i]))>0){
                                                                                                        $grupito=grupo($vector_equipo_reemplazo[$i]);

                                                                                                        $cadena = trim(trim($vector_equipo_reemplazo[$i]).trim($vector_modelo_retirado[$i]).trim($vector_serie_retirado[$i]).trim($vector_modelo_reemplazo[$i]).trim($vector_serie_reemplazo[$i]).trim($vector_observaciones[$i]).$tipo_formulario);
                                                                                                        $cadena2 = trim(trim($vector_equipo_reemplazo[$i]).trim($vector_modelo_retirado[$i]).trim($vector_serie_retirado[$i]).trim($vector_modelo_reemplazo[$i]).trim($vector_serie_reemplazo[$i]).trim($vector_observaciones[$i]).$tipo_formulario);
                                                                                                        // echo $cadena2;
                                                                                                        if(!in_array($cadena, $vector) && strlen(trim($cadena)) > 0){
                                                                                                                $vector[] = $cadena;
                                                                                                                echo $abrir_fila;
                                                                                                                echo $contenido_tabla;
                                                                                                                echo "<td>".utf8_decode($grupito)."</td>";
                                                                                                                echo "<td>".utf8_decode($vector_equipo_reemplazo[$i])."</td>";
                                                                                                                echo "<td>SI</td>";
                                                                                                                echo "<td>".utf8_decode($vector_modelo_retirado[$i])."</td>";
                                                                                                                echo "<td>".utf8_decode($vector_serie_retirado[$i])."</td>";
                                                                                                                echo "<td>".utf8_decode($vector_modelo_reemplazo[$i])."</td>";
                                                                                                                echo "<td>".utf8_decode($vector_serie_reemplazo[$i])."</td>";
                                                                                                                echo "<td>".utf8_decode($vector_observaciones[$i])."</td>";
                                                                                                                echo $contenido_tabla2;
                                                                                                                echo $cerrar_fila;
                                                                                                        }
                                                                                                // }
